...
AUKRO_RUNTIME_SECRET_FILENAMES = {".env", ".env.backup", ".env.backup-20260415T141408Z-unified-payments", ".env.backup.20260307121224"}
...
def scan_sensitive_data(root: Path) -> list[dict[str, str]]:
    ...
    for path in sorted(root.rglob("*")):
        if not path.is_file():
            continue
        if path.name in AUKRO_RUNTIME_SECRET_FILENAMES:
            continue
