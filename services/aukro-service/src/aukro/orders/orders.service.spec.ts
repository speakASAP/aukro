...
async function run() {
  ...
  assert.match(replayEvent.eventId, /^aukro\.order-affinity:[0-9a-f]{32}$/);
  assert.equal(replayEvent.source, "aukro-service");
  assert.deepEqual(Object.keys(replayEvent.payload).sort(), ["channel", "currency", "items", "orderId"]);
  assert.match(replayEvent.payload.orderId, /^aukro-replay:[0-9a-f]{32}$/);
  assert.equal(replayEvent.payload.channel, "aukro");
  assert.equal(replayEvent.payload.currency, "CZK");
  assert.equal(replayEvent.payload.items.length, 2);
  assert.deepEqual(Object.keys(replayEvent.payload.items[0]).sort(), ["productId", "quantity", "sku", "totalPrice", "unitPrice"]);
  assert.equal(replayEvent.payload.items[0].productId, catalogProductId);
  assert.equal(replayEvent.payload.items[1].productId, otherCatalogProductId);
