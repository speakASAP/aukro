...
The task adds `docs/16_operations/AUKRO_PLATFORM_RULES.md`, TASK-011 IPS artifacts, graph/task tracker links, validation evidence, and a source-linked comment in `OfferPolicyService`. It does not add live Aukro mutation, browser automation, crawler code, WebAPI calls, schema changes, secrets, or deployment changes.
...
Vision -> `docs/01_vision/VISION.md` -> Feature -> `docs/10_features/FEAT-004-aukro-compliance-policy.md` -> Task -> `docs/11_tasks/TASK-011-document-aukro-platform-rules.md` -> Execution Plan -> `docs/21_execution_plans/EP-TASK-011-document-aukro-platform-rules.md` -> Coding Prompt -> `docs/14_prompts/PROMPT-TASK-011-document-aukro-platform-rules.md` -> Code comment -> Validation -> `docs/12_validation/VAL-TASK-011-document-aukro-platform-rules.md`.
