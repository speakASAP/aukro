...
See `docs/22_goal_impact/GOAL-IMPACT-TASK-015.md`. This task gives the team a controlled feed-based fallback for catalog publication while official API credentials remain blocked or incomplete.
...
Use `docs/21_execution_plans/EP-TASK-015-mp-xml-feed-executor.md` before coding. The plan does not approve live MP import activation; it only defines a safe, testable fallback lane.
